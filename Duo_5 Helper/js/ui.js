...
function rankings() {
  ...
  if (isDuo) {
    ...
    <div>
      ...
      <div class="rank-grid" style="grid-template-columns:1fr 1fr">
        ${row('S',n('ten_pair'),'10+10 · JangTtaeng',tx('nAllStd'),'tag-g')}
        ${row('S',n('judge'),'3+7',tx('nPairs'),'tag-g')}
        ${row('S',n('warden'),'4+9',tx('nRematch2'),'tag-g')}
      </div>
      <div class="rank-sec">${tx('sRank')}</div>
      <div class="rank-grid" style="grid-template-columns:1fr 1fr">
        ${row('1',n('ten_pair'),'10+10 · JangTtaeng',tx('nAllStd'),'')}
        ${row('2',fr?'Paires 9→1':'Pairs 9→1',fr?'Plus haute gagne · Ttaeng':'Higher wins · Ttaeng','','')}
        ${row('3',n('one_two'),'1+2','','')}
        ...
        ${row('9',n('perfect_nine'),fr?'Somme finit en 9 · GapO':'Sum ends 9 · GapO','','')}
        ${row('10',fr?'Points 8→1':'Points 8→1',fr?'Dernier chiffre · Ggut':'Last digit · Ggut','','')}
        ${row('11',n('zero'),fr?'Somme finit en 0 · MangTong':'Sum ends 0 · MangTong','','')}
      </div>
    </div>
    <div>
    ...
    body = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
      <div>
        ${sec(fr?'Combinaisons Standard':'Standard Combinations',
          rowDesc('1',n('ten_pair'),'10 + 10 · JangTtaeng', fr?'Bat toutes les autres mains standards.':'Beats every other standard hand.','')+
          rowDesc('2',fr?'Paires 9→1':'Pairs 9→1', fr?'Toute paire identique · Ttaeng':'Any matching pair · Ttaeng', fr?'Plus élevée gagne.':'Higher pair wins.','')+
          rowDesc('3',n('perfect_nine'),fr?'Somme = 9 · GapO':'Sum = 9 · GapO', fr?'Main la plus forte après les Paires.':'Strongest hand after Pairs.','')+
          rowDesc('4',fr?'Points 8→1':'Points 8→1', fr?'Chiffre des unités de la somme · Ggut':'Units digit of the total sum · Ggut', fr?'Le plus élevé gagne.':'Highest wins.','')+
          rowDesc('5',n('zero'),fr?'Somme = 0 · MangTong':'Sum = 0 · MangTong', fr?'Hors mains supérieures.':'Outside higher-ranked hands.','')+
          rowDesc('6',fr?'Échec':'Bust', fr?'Aucune combo 10/20/30 · Hwang':'No valid 10/20/30 · Hwang', fr?'Défaite automatique.':'Automatic loss.','')
        )}
      </div>
      <div>
