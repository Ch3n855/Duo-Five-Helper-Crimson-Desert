...
Situation toggle — you can tell the calculator whether you're betting first or responding to a raise, and it adjusts the advice accordingly
...
Hand rules that were wrong or missing
The Judge hand (3+7) didn't exist. In the original, if you held a 3 and a 7, the game silently treated your hand as Zero — the weakest possible result. The Judge hand was simply never coded. It's now fully implemented: it beats all Pairs, but is treated as Zero against everything else.
...
What the odds calculation got wrong — and was fixed
The original calculated your odds against an imaginary full deck of 20 cards every time. The problem: it included cards that were already on the table. For example if your opponent was showing a Red 9, the calculator still counted "Red 9 + Red 9" as a possible hand for them — which is physically impossible since there's only one Red 9 in the deck. The fix removes all known cards from the pool before calculating, giving accurate numbers.
The original also had a fourth outcome called "Rematch" which sat separately from wins and losses. This quietly shrank the displayed win and loss percentages. The current version folds Rematch into Tie, so the percentages actually add up to what matters for your betting decision.
